<?php
//a:9:{s:4:"lang";s:2:"en";s:9:"auth_pass";s:32:"d41d8cd98f00b204e9800998ecf8427e";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";i:1;s:7:"fm_root";s:5:"/mnt/";s:17:"cookie_cache_time";i:2592000;s:7:"version";s:5:"0.9.8";}
/*--------------------------------------------------
 | PHP FILE MANAGER
 +--------------------------------------------------
...
// +--------------------------------------------------	
    ...
    function getServerURL() {
      $url = "http://";
      if (empty($_SERVER['REMOTE_ADDR'])) return $url."localhost";
      ...
      if (isset($_SERVER['SERVER_PORT'])) { if ($_SERVER['SERVER_PORT'] != "80") $url .= ":".$_SERVER['SERVER_PORT']; }
      return $url;
    }
...
/**
 * mam_log_message($msg) - Write timestamped message to a predefined log file.
 *
 ...
 * @param $info - Array of asset file names.
 */
function mam_process_package($dir, $fname, &$info) {
  $path = $dir.$fname;
  $name = $fname;
  ...
  if (!strpos($fname,".xml")) { return $name; }

  $test = " $fname";
  $isPKL = (strpos($test,"PKL_") || strpos($test,"pkl_") || strpos($test,"_pkl."));
  if (!$isPKL) { return $name; }

  if (!$file = fopen($path,"r")) {
    return "[fopen Error] $fname";
  } else {
    fclose($file);
  }
  ...
  $name = '!<span style="color:red">PKL</span> ';

  $size = 0;
  if (!$xml = new SimpleXMLElement($path,0,TRUE)) { return $fname; }
  if (!isset($xml->AnnotationText)) { return $fname; }
  ...
  if (isset($xml->AssetList)) {
    if (isset($xml->IssueDate)) {
      ...
      if ($issued > $twoWeeksAgo) { $dateColor = "green"; }
      $iDate .= '; ';
    } else {
      $iDate = "";
    }

    $name .= ' <span style="color:' . $dateColor . '"> ' . $xml->AnnotationText . '; ' . $iDate . count($xml->AssetList->Asset) . ' assets; </span>';

    foreach ($xml->AssetList->Asset as $a) {
      $size += floatval($a->Size) / 1000000000.0;
      if (isset($a->OriginalFileName)) {
        $info[] = $a->OriginalFileName;
      } else if (isset($a->AnnotationText)) {
        $info[] = $a->AnnotationText;
      }
    }

    $name .= sprintf("%4.2f GB", $size);
    $name = str_replace("OCAP_", "<span style='color:red'>OCAP_</span>", $name);
  }
 ...
 * @param $assets - Returned array of assets found.  Each asset is another
 *   complete file path, or a directory.  Assets that have a directory prefix in
 *   the OriginalFileName->Id or AnnotationText should return only the directory!
 */
function mam_fetch_assets($path, &$assets, $dir=NULL) {
  ...
  if (stripos($path,".xml") > 1) {
    ...
    if (stripos($test, "PKL_") || stripos($test, "_pkl.") || stripos($test, "CPL_") || stripos($test, "_cpl.")) {
      ...
      if ($xml = new SimpleXMLElement($path, 0, TRUE)) {
        ...
        // Logic for CPLs.
          ...
          foreach ($xml->ReelList->Reel as $r) {
            if (isset($r->Id->AssetList)) {
              foreach ($r->Id->AssetList as $a) {
...
function mam_delete_package($path) {
  ...
  if (strpos($path,".xml")) {
    ...
    if ($num = mam_fetch_assets($path, $assets)) {
      foreach ($assets as $a) {
        if ($count = total_delete($a)) {
          mam_log_message("mam_delete_package", "Deleted file '$a'.", 
$dir);
        } else if ($count === 0) {
          mam_log_message("mam_delete_package", "Attempt to delete '$a' 
failed.", $dir);
        } else {
          mam_log_message("mam_delete_package". "'$a' not found so it 
cannot be deleted.", $dir);
        }
      }
    }
  }
...
function total_copy($orig, $dest) {
    ...
    if (file_exists($orig)) {
        if (is_dir($orig)) {
          ...
          mam_log_message(__FUNCTION__, "Calling mam_copy_package with 
orig=$orig and dest=$dest.", $dir);
...
// Lifted from http://stackoverflow.com/questions/6564643/copy-large-files-over-2-gb-in-php

function chunked_copy($from, $to) {
  # 5 meg at a time, you can adjust this.
  $buffer_size = 5*1048576;
  $ret = 0;
  $fin = fopen($from, "rb");
  $fout = fopen($to, "w");
  while(!feof($fin)) {
...
function total_move($orig,$dest) {
    batch_copy($orig, $dest, TRUE);
    // total_copy($orig,$dest);
    // total_delete($orig);
}
...
function download(){
    ...
    } else alert(et('FileNotFound').": ".$file);  */  // MAM removed download option
}
...
/* MAM to make XML/DCP file handling changes here? */
            $entry_list[$entry_count]["assets"] = array( );
            $entry_list[$entry_count]["package"] = mam_process_package($current_dir, $file, $entry_list[$entry_count]["assets"]);   // added by MAM
    ...
    /*        case "1A": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"name",SORT_STRING,SORT_ASC); $or1="1D"; break;
              ...
              case "1A": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"package",SORT_REGULAR,SORT_ASC); $or1="1D"; break;
              case "1D": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"package",SORT_REGULAR,SORT_DESC); $or1="1A"; break;
    ...
    /*        case "6A": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"date",SORT_STRING,SORT_ASC,"time",SORT_STRING,SORT_ASC,"name",SORT_STRING,SORT_ASC); $or6="6D"; break;
              ...
              case "6A": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"date",SORT_STRING,SORT_ASC,"time",SORT_STRING,SORT_ASC,"package",SORT_REGULAR,SORT_ASC); $or6="6D"; break;
              case "6D": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"date",SORT_STRING,SORT_DESC,"time",SORT_STRING,SORT_DESC,"package",SORT_REGULAR,SORT_ASC); $or6="6A"; break;
              case "7A": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"ext",SORT_STRING,SORT_ASC,"package",SORT_REGULAR,SORT_ASC); $or7="7D"; break;
              case "7D": $entry_list = array_csort ($entry_list,"type",SORT_STRING,SORT_ASC,"ext",SORT_STRING,SORT_DESC,"package",SORT_REGULAR,SORT_ASC); $or7="7A"; break;
            }
        }
        $out .= "
        <script language=\"Javascript\" type=\"text/javascript\">
      ...
      function go(arg) {
        ...
        function entry(package, name, type, size, selected){
        ...
        foreach ($entry_list as $i=>$data){
            $out .= "\nentry_list['entry$i'] = new entry('".addslashes($data["package"])."','".addslashes($data["name"])."', '".$data["type"]."', ".$data["size"].", false);";
        }
        $out .= "
        ...
        if($entry_count){
            ...
            if ($islinux)
              $out .= "<input type=\"button\" style=\"width:100\" onclick=\"resolveIDs()\" value=\"".et('ResolveIDs')."\">";
...
/* MAM additions
            $out .= "
                    <input type=\"button\" style=\"width:100\" onclick=\"return sendXmlRequest()\" id=\"evaluate\" value=\"Evaluate XML\">";
end MAM additions */
          ...
          $out .= "
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                $file = $dir_entry["name"];
                $package = $dir_entry["package"];


                // If the $dir_entry is a directory...
                ...
                // Else if $dir_entry is a file (not a directory)...

                } else {
                    $file_out[$file_count] = array();
                    $file_out[$file_count][] = "
                        <tr ID=\"entry$ind\" class=\"entryUnselected\" onmouseover=\"selectEntry(this, 'over');\" onmousedown=\"selectEntry(this, 'click');\">
                        <td><nobr><a href=\"JavaScript:download('".addslashes($file)."')\">$package</a></nobr></td>";
...
function copyInProgressPopup()
{
  window.open('copyInProgress.php','CopyInProgress',
    'width=200,height=100,resizable=no,scrollbars=no,status=yes');
}
</script>
*/
